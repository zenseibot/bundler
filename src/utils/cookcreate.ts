...
// Interfaces
export interface WalletForCookCreate {
  publicKey: string;
  privateKey: string;
...
export interface TokenMetadata {
  ...
  twitter?: string;
  website?: string;
  discord?: string; // Cook.meme has discord field
}

export interface CookCreateConfig {
  tokenMetadata: TokenMetadata;
  ownerPublicKey: string;
  initialBuyAmount: number; // SOL amount for initial buy
  tradingTimestamp?: number; // Optional timestamp for when trading starts
...
/**
 * Get partially prepared cook create transactions from backend
 */
const getPartiallyPreparedCookTransactions = async (
  config: CookCreateConfig,
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/cookmeme/create`, {
      ...
      body: JSON.stringify({
        ...
        tradingTimestamp: config.tradingTimestamp || 0,
        settingsVersion: config.settingsVersion || 1
      }),
    });
...
const decodeTransaction = (transactionStr: string): Uint8Array => {
  // Check if the transaction is already base64 encoded (cook.meme format)
 ...
 */
export const executeCookCreate = async (
  config: CookCreateConfig,
