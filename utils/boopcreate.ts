...
// Interfaces
export interface WalletForBoopCreate {
  address: string;
  privateKey: string;
}
...
/**
 * Get partially prepared boop create transactions from backend
 */
const getPartiallyPreparedTransactions = async (
  ...
  try {
    ...
    const data: BoopCreateResponse = await response.json();
    ...
    const bundles: BoopCreateBundle[] = [];
...
/**
 * Execute boop create operation on the frontend with improved reliability
 */
export const executeBoopCreate = async (
  wallets: WalletForBoopCreate[],
  tokenCreationConfig: TokenCreationConfig,
 ...
 */
const sendFirstBundle = async (bundle: BoopCreateBundle): Promise<{success: boolean, result?: any, error?: string}> => {
